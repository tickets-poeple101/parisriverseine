...
type GalleryItem =
  ...
  function Stars({
  value,
  size = 24,                      // ← change size here for bigger/smaller stars
  colorClass = "text-slate-800",  // ← navy/dark fill for full/half stars
}: {
  value: number;
  size?: number;
  colorClass?: string;
  ...
  const uid = useId(); // unique base for clipPath ids

  return (
    <div
      className={`flex items-center gap-3 ${colorClass}`}
      aria-label={`${value} out of 5`}
    >
      {Array.from({ length: 5 }).map((_, i) => {
        const diff = rounded - i;
        const commonSvgProps = { viewBox: "0 0 20 20", "aria-hidden": true as const, style: { width: size, height: size } };
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="relative h-2 flex-1 rounded-full bg-slate-200">
        <div className="absolute inset-y-0 left-0 rounded-full bg-emerald-500" style={{ width: `${(value / 5) * 100}%` }} />
      </div>
...
export default function ReviewsSection() {
  // DATA (swap with local files when ready — see notes below)
  const overall = 4.4;
  ...
  const gallery: GalleryItem[] = [
    {
      kind: 'video',
      src: '/reviews/hero.mp4',
      poster: '/reviews/hero.jpg',
      alt: 'Cruise highlight video',
    },
    ...
    <section aria-labelledby="reviews-title" className="mx-auto max-w-6xl px-4 py-12">
      <div className="mb-6 flex items-center gap-2">
        ...
        <span className="text-slate-400" title="Reviews are from verified bookings.">
          <svg className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path fillRule="evenodd" d="M18 10A8 8 0 112 10a8 8 0 0116 0zM9 7a1 1 0 112 0 1 1 0 01-2 0zm-1 7a1 1 0 100-2h1V9h2a1 1 0 100-2H9a1 1 0 00-1 1v4H7a1 1 0 100 2h1z" clipRule="evenodd" />
          </svg>
        </span>
      </div>
      ...
      <div className="isolate overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-inset ring-slate-300">
        <div className="grid grid-cols-4 md:grid-cols-5 md:grid-rows-2 md:auto-rows-[220px] gap-0">
          ...
          >
            <img src={(gallery[0] as any).poster} alt="Video poster" className="block h-full w-full object-cover" loading="lazy" />
          ...
          {gallery.slice(1, 5).map((g, i) => (
            <button
              ...
              aria-label={g.alt}>
              <img src={(g as any).src} alt={g.alt} className="block h-full w-full object-cover" loading="lazy" />
              {i === 3 && (
      ...
      <div className="mt-8 grid gap-8 md:grid-cols-2">
        <div>
          ...
          <div className="mt-2">
            ...
            <div className="mt-1 text-sm text-slate-500">based on {new Intl.NumberFormat('en-US').format(reviewCount)} reviews on GetYourGuide</div>
          </div>
        ...
        <div className="space-y-6">
          {[
            { id: 'r1', name: 'Jocelyn', country: 'Canada', initials: 'J', rating: 4.5, dateLabel: 'October 13, 2025 — Verified booking', text: "The Seine River cruise was a lovely way to see the city! Views were stunning and the guide’s info was perfect. Meeting point was easy to find. Note: there aren’t beverages for purchase on the boat. Still really enjoyed our experience!" },
            { id: 'r2', name: 'Laura', country: 'United Kingdom', initials: 'L', rating: 5, dateLabel: 'October 13, 2025 — Verified booking', text: 'One-hour scenic cruise was delightful from start to finish. Gliding past landmarks gave a new appreciation of Paris. Commentary (multiple languages) was just the right amount.' },
            { id: 'r3', name: 'Samuel', country: 'United Kingdom', initials: 'S', rating: 5, dateLabel: 'October 12, 2025 — Verified booking', text: 'Very knowledgeable guide. Impressed that she spoke three languages. Incredible views and smooth ride.' },
          ].map((r, i) => (
            <article key={r.id} className="rounded-xl border border-slate-200 p-4 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  <span className={`grid h-9 w-9 place-items-center rounded-full text-sm font-semibold ${colorFor(r.initials)}`} aria-hidden="true">
                    {r.initials}
                  </span>
                  <div>
      ...
      {open && (
        ...
        >
          <div className="absolute inset-0 flex items-center justify-center p-3 md:p-6" onClick={(e) => e.stopPropagation()}>
            {/* media */}
            <div className="relative max-h-full w-full max-w-5xl">
              <button
                ...
                aria-label="Close"
              >
                <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6l12 12M18 6L6 18" /></svg>
              </button>
              ...
              <button
                ...
                aria-label="Previous"
              >
                <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M15 18l-6-6 6-6" /></svg>
              </button>
              <button
                onClick={next}
                ...
                aria-label="Next"
              >
                <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M9 6l6 6-6 6" /></svg>
              </button>
              ...
              <div className="mx-12">
                {gallery[idx].kind === 'image' ? (
                  ...
                  <video
                    src={gallery[idx].src}
                    poster={(gallery[idx] as any).poster}
                    controls
                    autoPlay
                    muted
                    playsInline
